...
**Исходный архитектурный срез:** 13 июля 2026 года. Цены, возможности провайдеров и правовые требования требуют повторной проверки перед внедрением.
...
Исходный срез документа подготовлен после PR #66. При завершении этого handoff в `main` также объединены:

- PR #70 — план готовности к публикации в RuStore;
- PR #72 — клиентская проекция `approved_caregiver_profiles` возвращает `NULL` вместо телефона сиделки; адресная выдача контактов ещё требует отдельного workflow назначения и аудита.

Объединение миграции в Git не подтверждает её применение к рабочей базе. Российский staging, hosted Auth и production deployment этим документом не объявляются проверенными.
...
Для проектируемой электронной письменной формы предусмотреть формализованную простую электронную подпись: идентифицированный аккаунт/телефон, отдельное соглашение о ПЭП, OTP, точный snapshot документа, версия, hash, время, подписант и доказательство проверки. Аккаунт, OTP и snapshot сами по себе не подтверждают юридическую достаточность подписи. Применимость ПЭП, содержание соглашения и всю схему письменного согласия до реализации должен проверить российский профильный юрист; этот checklist согласован с разделом 8 `docs/rustore-release-readiness.md`.
